...
class ProductionDependenciesContainerFactory
{
    ...
    {
        return new DependenciesContainer(new ContainersFactory(neblabs_collection([$this->dependenciesContainerContainerFactory(), $this->dependencyContainersFactory($dependencyTypes->get()), $this->automaticDependentContainerFactory(), $this->unresolvableDependencyContainerFactory()])), new OverloadedContextFactory(new FactoryOverloader(neblabs_collection([new PassThroughContextFactory(), new UnknownContextFactory()]))));
    }
    ...
    {
        return new DependencyContainersFactory($dependencyTypes, new DependencyTypeToContainerFactory(new DependencyFactory(new DependencyInspectorFactory(), new DependentFactory($this->dynamicTypeFactory(), new OverloadedContextFactory(new FactoryOverloader(neblabs_collection([new KnownContextFactory(), new UnknownContextFactory()]))))), new DependencyContainerFactory(new DependencyInspectorFactory())));
    }
    protected function automaticDependentContainerFactory() : CreatableContainers
    {
        return new AutomaticDependencyContainerFactory($this->dynamicTypeFactory());
    }
