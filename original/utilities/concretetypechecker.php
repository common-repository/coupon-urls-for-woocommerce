...
class ConcreteTypeChecker
{
    ...
    public function toBeBoolean()
    {
        ...
        throw new UnexpectedTypeException(\esc_html("Value must be boolean, " . gettype($this->value) . ' given.'));
    }
    protected function checkSingleValue($type)
    {
        if (!is_a($this->value, $type, $useClassNames = true)) {
            ...
            throw new UnexpectedTypeException(\esc_html("{$givenValue} must be instance of {$type}"));
        }
        return $this->value;
    }
