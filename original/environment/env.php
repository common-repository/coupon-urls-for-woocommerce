...
class Env
{
    ...
    public static function getEncoderFunction() : callable
    {
        (string) ($base = 'json_encode');
        switch (function_exists("wp_{$base}")) {
            case true:
                return "wp_{$base}";
            case false:
                return $base;
     ...
     * @return mixed
     */
    public static function for(Collection $environmentCallables)
    {
        $callable = $environmentCallables->find(function ($value, string $environment) {
            return $environment === static::settings()->environment;
        });
        if ($callable) {
            return $callable;
        }
    ...
    {
        (string) ($convertedPath = StringManager::create($path)->trim('/')->explode('/')->map(function (string $pathPart) : string {
