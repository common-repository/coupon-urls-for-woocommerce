...
class EventsRegistrator
{
    ...
    public function __construct()
    {
        ...
        $this->customEvents = new Collection(require_once Env::directory() . "/app/{$eventsFile}");
        $this->originalEvents = new Collection(require_once Env::directory() . "/original/{$eventsFile}");
    }
    public function register()
    {
        foreach ($this->getAllEvents()->asArray() as $eventName => $eventHandlers) {
            $this->registerHandlersFor(['name' => (string) $eventName, 'handlers' => (array) $eventHandlers]);
        }
    ...
    protected function getAllEvents()
    {
        ...
        return $events->mapwithKeys(function (array $handlers, string $eventName) : array {
            return ['key' => $this->replacePlaceHolders($eventName), 'value' => $handlers];
        });
    }
    protected function replacePlaceHolders(string $text) : string
    {
        (object) ($placeholders = new Collection(['(id)' => Env::id(), '(shortId)' => Env::shortId()]));
        return str_replace($placeholders->getKeys()->asArray(), $placeholders->getValues()->asArray(), $text);
    }
}