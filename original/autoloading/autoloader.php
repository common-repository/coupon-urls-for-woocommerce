...
class Autoloader
{
    ...
    public static function register()
    {
        spl_autoload_register(function ($name) {
            try {
                ...
                throw new AutoloaderException(\esc_html("Error when trying to load class: {$name}. Error message: {$exception->getMessage()} in: {$exception->getFile()}:{$exception->getLine()}\n\n"));
            }
        });
    }
    public function __construct($fullyQualifiedClassName)
    {
        ...
        (int) ($classNameindex = count($namespaces) - 1);
        (array) ($lowerCaseNamespaces = array_map('lcfirst', $namespaces));
        (array) ($capitalizedClass = ucfirst($lowerCaseNamespaces[$classNameindex]));
    ...
    {
        (bool) ($firstNamespace = 0);
        return strpos(strtolower($this->fullyQualifiedClassName), strtolower("{$this->masterNamespace}\\")) === $firstNamespace;
    }
    protected function classFileName()
    {
        (string) ($classNameWithNoMasterNamespace = substr($this->fullyQualifiedClassName, strlen($this->masterNamespace) + 1));
