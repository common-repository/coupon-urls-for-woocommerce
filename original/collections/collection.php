...
class Collection implements IteratorAggregate, ArrayRepresentation, JsonSerializable
{
    ...
    {
        $this->setElements(ArrayGetter::getArrayOrThrowExceptionFrom($elements ?? []));
    }
    /**
    ...
    {
        $this->setElements(array_merge([$element], is_array($this->elements->getArrayCopy()) ? $this->elements->getArrayCopy() : iterator_to_array($this->elements->getArrayCopy())));
        return $this;
    }
     ...
     * @param mixed $value
     */
    public function setIfDoesNotHave(string $key, $value) : self
    {
        if (!$this->hasKey($key)) {
            $this->set($key, $value);
        }
    ...
    public function ungroup() : self
    {
        ...
        foreach ($this->elements as $key => $value) {
            if (ArrayGetter::isArrayRepresentation($value)) {
                $brokenItems = static::create($value)->map(function ($item) use ($key) {
                    return new static([$key => $item]);
                });
        ...
        foreach ($this->elements as $value) {
            if (ArrayGetter::isArrayRepresentation($value)) {
                ...
                if (!$groups->hasKey($item->firstKey())) {
                    $groups->add($item->firstKey(), new static([]));
                }
    ...
    public function first($numberOfItems = null)
    {
        if (!is_null($numberOfItems)) {
            (array) ($elements = (array) $this->elements);
    ...
    {
        [$methodName, $methodValue] = $this->getMethodNameAndValueFromVariableParameters(...$methodAsKeyAndValueAsTheMethodsValue);
        //dr(debug_backtrace(limit: 10));
        return $this->map(function ($item) use ($methodName, $methodValue) {
            return is_array($item) ? $item[$methodName] : $item->{$methodName}($methodValue);
        });
    }
    ...
    {
        [$methodName, $methodValue] = $this->getMethodNameAndValueFromVariableParameters(...$methodAsKeyAndValueAsTheMethodsValue);
        return $this->forEvery(function ($item) use($methodName, $methodValue) : bool {
    ...
    {
        [$methodName, $methodValue] = $this->getMethodNameAndValueFromVariableParameters(...$methodAsKeyAndValueAsTheMethodsValue);
        return $this->doesNotHave(function ($item) use ($methodName, $methodValue) {
            return !$item->{$methodName}($methodValue);
        });
    }
    /**
     * @return \CouponURLs\Original\Characters\StringManager|string|int|float|$this
     */
    ...
    {
        [$methodName, $methodValue] = $this->getMethodNameAndValueFromVariableParameters(...$methodAsKeyAndValueAsTheMethodsValue);
        return $this->filter(function (object $item) use ($methodName, $methodValue) {
            return $item->{$methodName}($methodValue);
        });
    }
    /**
     * @return mixed
     */
    public function findTheOneThat(...$methodAsKeyAndValueAsTheMethodsValue)
    {
        [$methodName, $methodValue] = $this->getMethodNameAndValueFromVariableParameters(...$methodAsKeyAndValueAsTheMethodsValue);
        return $this->find(function (object $item) use ($methodName, $methodValue) {
            return $item->{$methodName}($methodValue);
        });
    }
    /**
    ...
    {
        if (!is_callable($callable)) {
            $callable = \Closure::fromCallable('array_filter');
        }
        return $this->getFilteredElements(function ($element, $key) use($callable) {
            (bool) ($shouldBeFiltered = $callable instanceof Closure || $callable instanceof Invokable ? $callable($element, $key) : $callable($element));
            if ($shouldBeFiltered) {
                $this->remove($key);
                return true;
            }
    ...
    protected function checkHave($value, bool $strictComparison = false) : self
    {
        return $this->filter(function ($valueToCompareAgainst) use($value, $strictComparison) : bool {
            ...
            if ($value instanceof Comparable) {
                ...
                if ($valueToCompareAgainst instanceof Comparable) {
                    return $valueToCompareAgainst->isTheSameAs($value);
                }
            }
            (array) ($values = [[$value, $valueToCompareAgainst], [$valueToCompareAgainst, $value]]);
    ...
    protected function getFilteredElements(callable $callable, $limit = 0) : self
    {
        ...
        foreach ($this->asArray() as $key => $value) {
            if ($callable instanceof Closure || $callable instanceof Invokable) {
                (bool) ($canBeIncluded = $callable($value, $key));
            } else {
                (bool) ($canBeIncluded = $callable($value));
            }
            if ($canBeIncluded) {
    ...
    ) : self
    {
        ...
        return new static(array_intersect_key($this->asArray(), array_flip($keysToInclude)));
    }
    /**
    ...
    public function contain($itemToSearch) : bool
    {
        ...
        if (is_array($itemToSearch) || $itemToSearch instanceof Collection) {
            ...
            $itemToSearch = (new static($itemToSearch))->map(static::convertToString())->asArray();
        }
        return in_array($itemToSearch, $this->map(static::convertToString())->map(function ($value) {
            return $value instanceof Collection ? $value->asArray() : $value;
        })->asArray(), $strictTypeSearch = true);
    }
    //checks that the given collection exists within this collection
    //INCLUDING THE KEYS
    /**
     * @param mixed[]|\CouponURLs\Original\Collections\Collection $collection
     */
    public function containCollection($collection) : bool
    {
    ...
    /**
     * @param mixed[]|\CouponURLs\Original\Collections\Collection $elements
     */
    public function containAny($elements) : bool
    {
        (array) ($elements = ArrayGetter::getArrayOrThrowExceptionFrom($elements));
    ...
    ) : bool
    {
        ...
        if (empty($elements) && $this->haveAny()) {
            return false;
        }
        foreach ($elements as $element) {
            if (!$this->contain($element)) {
    ...
    {
        foreach ($this->asArray() as $element) {
            (bool) ($hasItpassed = $callable($element) === true);
            if ($hasItpassed) {
                return true;
            }
        }
    ...
    public function lastKey()
    {
        if (function_exists('array_key_last')) {
            // better performance on big arrays
    ...
    {
        reset($methodAsKeyAndValueAsTheMethodsValue);
        (bool) ($passedANamedParameter = is_string(key($methodAsKeyAndValueAsTheMethodsValue)));
        if ($passedANamedParameter) {
            $methodAsKeyAndValueAsTheMethodsValue[0] = $methodAsKeyAndValueAsTheMethodsValue;
        }
        reset($methodAsKeyAndValueAsTheMethodsValue[0]);
        (string) ($methodName = key($methodAsKeyAndValueAsTheMethodsValue[0]));
        /*mixed*/
        $methodValue = $methodAsKeyAndValueAsTheMethodsValue[0][$methodName];
