...
class JSONMapper
{
    ...
    public static function sanitizeValue(
        /*any*/
        $value,
        callable $customSanitizationCallable = null
    )
    {
    ...
    /**
     * @param mixed[]|\CouponURLs\Original\Collections\Collection $map
     */
    public function __construct($map, $securityMode = 'escape')
    {
    ...
    public function smartMap($value)
    {
        if (!$this->hasMap() && $this->isNonJson($value)) {
            ...
            (object) ($jsonObject = (object) $value);
            //$this->getObjectFromJson(Env::getEncoderFunction()($value));
        } else {
    ...
    public function mapObject($parameters)
    {
        ...
        foreach ($this->mapDefinition as $key => $value) {
            ...
            if (is_array($value)) {
                ...
                (array) ($newArray = is_string($parameterValue) ? Collection::createFromString($parameterValue, $value instanceof CollectionType ? $value->getSeparator() : null) : (array) $parameterValue);
                ...
                $parameters->{$key} = new StringManager(static::sanitizeValue($stringValue, $this->getCustomSanitizationFunction($value)));
                ...
                if ($this->valueIs($value, Types::INTEGER)) {
                    $earlyValue = is_integer($parameterValue) || is_numeric($parameterValue) ? (int) $parameterValue : 0;
                ...
                if ($value instanceof Types && $value->hasMinimum()) {
                    (int) ($minimumValue = $value->getOrDefaultToMinimum($earlyValue));
                    $parameters->{$key} = $earlyValue >= $value->getMinimum() ? $minimumValue : max($minimumValue, (int) $value->getDefaultValue());
                } else {
    ...
    protected function removeEntriesNotInMap($parametersObject)
    {
        ...
        $newObject->allFieldsFoundInSource->append($sourceObject->getKeys()->filter(S::stringsOnly())->map(function ($property) {
            return new StringManager($property);
        })->asArray());
        $newObject->setRawDataFound(new Collection(array_intersect_key((array) $parametersObject, (array) $this->mapDefinition)));
        return $newObject;
    }
    ...
    protected function getEscapeFunction($mapValue)
    {
