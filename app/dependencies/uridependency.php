...
class URIDependency implements Cached, StaticType, Dependency
{
    ...
    public function create() : object
    {
        ...
        (object) ($uriForCurrentRequestFactory = new OverloadedEntitiesFactory(neblabs_collection([new HomePageURIFactory($queryParametersFactory), new PathURIFactory($queryParametersFactory)])));
        (object) ($globalRequest = Request::createFromGlobals());
        (object) ($homePath = i(wp_parse_url(get_home_url(), PHP_URL_PATH))->ensureRight('/'));
        (object) ($serverPath = i(($globalRequest->getPathInfo() ?? $globalRequest->getRequestUri()) ?? '/')->ensureRight('/'));
        /**
        ...
        return $uriForCurrentRequestFactory->createEntity(neblabs_collection([wp_parse_url(Request::create($serverPath->get(), 'GET', $globalRequest->query->all())->getUri())]));
    }
}