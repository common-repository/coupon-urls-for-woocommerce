...
class DahsboardURLsExporter implements DashboardExportable
{
    ...
    {
        (object) ($homepageUrlParts = neblabs_collection([wp_parse_url(get_home_url())]));
        return ['homepage' => get_home_url(), 'homepageNoProtocol' => i("{$homepageUrlParts->get('host')}/{$homepageUrlParts->get('path')}")->replace('//', '/')->trim()->trimRight('/')];
    }
}