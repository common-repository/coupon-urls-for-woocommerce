...
class QueryParametersFromStringFactory
{
    ...
    {
        return new QueryParameters(i($query)->removeLeft('?')->explode('&')->mapWithKeys(function (StringManager $keyAndValue) {
            return ['key' => (string) $keyAndValue->explode('=')->first()->toLowerCase(), 'value' => (string) $keyAndValue->contains('=') ? $keyAndValue->explode('=')->last()->toLowerCase() : ''];
        }));
    }
}